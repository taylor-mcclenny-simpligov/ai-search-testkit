"""Author: Taylor M
Run TM-MVP-1 dual-vector query and log results (separate log file prefix).

Usage:
  python scripts/experiments/tm_mvp1_query_log.py --dotenv \
    --input outputs/experiments/TM-MVP-1/tagged aas-60.json \
    --index simpligov-sgws-compressed --k 5 --field-limit 40

Logs:
  - logs/experiments/tm_mvp1_dual.jsonl (append)
  - logs/experiments/tm_mvp1_dual_pretty.json (append array)
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="TM-MVP-1 dual-vector query logger")
    ap.add_argument("--input", required=True, help="Path to tm_mvp1_extract JSON output")
    ...
    ap.add_argument("--log", default="logs/experiments/tm_mvp1_dual.jsonl", help="JSONL log path")
    ap.add_argument("--log-pretty", default="logs/experiments/tm_mvp1_dual_pretty.json", help="Pretty JSON array log path")
    args = ap.parse_args()
