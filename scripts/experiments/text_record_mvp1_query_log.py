"""Author: Taylor M
TM-MVP-1 style query against simpligov-text-records using service-side vectorization (kind=text).

Usage:
  python -m scripts.experiments.text_record_mvp1_query_log \
    --input outputs/experiments/TM-MVP-1/tagged_aas-60.json \
    --index simpligov-text-records --k 5 --dotenv

Logs to:
  outputs/query_logs/text_record_mvp1_pretty.json (appends array)
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="TM-MVP-1 text-records vector query logger")
    ap.add_argument("--input", required=True, help="Path to TM-MVP-1 extract JSON (semantics_text, field_candidates, headings_text optional)")
    ...
    ap.add_argument("--log-pretty", default="outputs/query_logs/text_record_mvp1_pretty.json", help="Pretty JSON array log path")
    args = ap.parse_args()
